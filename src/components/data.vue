    ...
    <div class="date-tools">
        ...
        <div class="drop-down-ym" v-if="showDropDownYM">
            <div class="drop-down-y">
                <a href="javascript:void(0);" v-for="(item,index) in yearList" :key="index" @click="selectY(item)">{{item}}</a>
            </div>
            <div class="drop-down-m">
                <a href="javascript:void(0);" v-for="(item,index) in monthList" :key="index" @click="selectM(item-1)">{{item}}</a>
            </div>
        </div>
    </div>
</template>
...
<script>
    export default ({
        ...
        watch: {
            panelMonth(newVal){//检测月份变化
                console.log(newVal)
                this.dateInit(this.panelYear,this.panelMonth);
            },
            panelYear(newVal){//检测年份变化
                console.log(newVal)
                this.dateInit(this.panelYear,this.panelMonth);
            },
            panelDay(newVal){//检测具体日期变化
