...
export default function formatDate(date, formatted) {
  ...
  if (typeof date === 'string') {
    ...
    if (formatted.match(/\bdate\b/gi)) {
      // string to Date
      return new Date(
        +date.replace(/(\d{4}).(\d{1,2}).?(\d{0,2})\s*(\d{0,2}).?(\d{0,2}).?(\d{0,2}).?/gm, function(it, y, M, d, h, m, s) {
          return new Date(y, M - 1, d, h, m, s).getTime()
        })
      )
    }
    if (formatted.match(/\b(number|DateTime)\b/gi)) {
      // string to DateTime
      return +date.replace(/(\d{4}).(\d{1,2}).?(\d{0,2})\s*(\d{0,2}).?(\d{0,2}).?(\d{0,2}).?/gm, function(it, y, M, d, h, m, s) {
        return new Date(y, M - 1, d, h, m, s).getTime()
      })
    }
